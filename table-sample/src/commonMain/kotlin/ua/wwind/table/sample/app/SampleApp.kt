...
fun SampleApp(modifier: Modifier = Modifier) {
    ...
    LaunchedEffect(state) {
        snapshotFlow { state.filters.toMap() }.collect { filters ->
            viewModel.updateFilters(filters)
        }
    }

    LaunchedEffect(state) {
        snapshotFlow { state.sort }.collect { sort -> viewModel.updateSort(sort) }
    }
            ...
            Column(
                ...
                Row(
                    ...
                    Row(
                        ...
                        Text("Fixed cols:")
                        OutlinedButton(
                                onClick = { if (fixedColumnsCount > 0) fixedColumnsCount-- }
                        ) { Text("-") }
                        Text("$fixedColumnsCount")
                        OutlinedButton(onClick = { fixedColumnsCount++ }) { Text("+") }
                    }
                    ...
                    Row(
                        ...
                        Switch(
                                ...
                                onCheckedChange = {
                                    fixedColumnsSide = if (it) FixedSide.Right else FixedSide.Left
                                },
                        )
                    }
